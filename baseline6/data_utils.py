...
obj_set = []
with open("../glove_vocab.txt",'r',encoding='utf8') as f:
    for line in f:
        word = line.strip()
        obj_set.append(word)
        ...
        with self._open_file(path) as f:
            for line in f:
                ...
                for idx,evt in enumerate(example['events']):
                    # text_words = example['text'].split(' ')
                    tmp_evt = ' '.join(evt)
                    if len(tmp_evt.split()) > 10:
                        continue
                    evt_q = evt
                    ...
                    #         continue
                    evt_k = evt_q
                    evt_n = [evt[0],evt[1],random.choice(obj_set)]
                    yield evt_q,evt_k,evt_n
...
class TrainData(tx.data.DatasetBase[Example, Example]):
    ...
    def process(self, raw_example):
        ...
        evt_q,evt_k,evt_n = raw_example[0], raw_example[1],raw_example[2]
        ...
        evt_n = ['<cls>']+[evt_n[0]] + [evt_n[1]] + [evt_n[2]]

        evt_n = ' '.join(evt_n).lower().split(' ')
        evt_n_ids = self._vocab.map_tokens_to_ids_py(evt_n)
    ...
    def collate(self, examples: List[Example]) -> tx.data.Batch:
        ...
        evt_n = [ex["evt_n"] for ex in examples]
        evt_n_ids, evt_n_lengths = tx.data.padded_batch(
            [ex["evt_n_ids"] for ex in examples], pad_value=self._vocab.pad_token_id)
        ...
        return tx.data.Batch(
            ...
            evt_n=evt_n,
            evt_n_ids = torch.from_numpy(evt_n_ids),
            evt_n_lengths = torch.tensor(evt_n_lengths),
        )
...
class HardData(tx.data.DatasetBase[Example, Example]):
    ...
    def process(self, raw_example):
        ...
        evt_d = ['<cls>']+[evt_d[0]] +[evt_d[1]] + [evt_d[2]]
        evt_d = ' '.join(evt_d).lower().split(' ')
        evt_d_ids = self._vocab.map_tokens_to_ids_py(evt_d)
    ...
    def collate(self, examples: List[Example]) -> tx.data.Batch:
        ...
        evt_d_ids, evt_d_lengths = tx.data.padded_batch(
            [ex["evt_d_ids"] for ex in examples], pad_value=self._vocab.pad_token_id)
...
class Vocab(tx.data.Vocab):
    def load(self, filename: str) \
        ...
        for w in [self._pad_token, self._bos_token, self._eos_token,
                 self._unk_token] + added_special_tokens:
            if w in vocab:
                raise BaseException(f"special token {w} has been already involved in Vocab...")
