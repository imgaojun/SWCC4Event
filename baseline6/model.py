...
class AdCo(nn.Module):
    ...
    def forward(self, batch):
        ...
        evt_n, evt_n_lengths = batch.evt_n_ids, batch.evt_n_lengths
        q = self.encoder_q(evt_q, evt_q_lengths)  # queries: NxC
        q = nn.functional.normalize(q, dim=1)

        # k = self.encoder_k(evt_k, evt_k_lengths)
        # k = nn.functional.normalize(k, dim=1)

        # n = self.encoder_k(evt_n, evt_n_lengths)
        ...
        return q, k, n
    # def predict_score(self, evt_q, evt_q_lengths):
    #     q = self.encoder_q(evt_q, evt_q_lengths)
    #     score = self.score_fc(q)
    #     return score
